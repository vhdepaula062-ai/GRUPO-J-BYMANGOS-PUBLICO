...
export async function GET(_request: NextRequest) {
  ...
  const { data, error } = await getPublicDatabase().from("promotions").select("id, title, description, image_url, discount_percentage, price_cents, start_date, end_date, workshop:organizations(id, trade_name)").eq("status", "active").lte("start_date", today).gte("end_date", today).order("end_date");
