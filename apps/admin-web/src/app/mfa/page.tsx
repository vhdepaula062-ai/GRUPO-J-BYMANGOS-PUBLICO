  ...
  const initMfa = useCallback(async () => {
    ...
    try {
      ...
      if (verifiedFactor) {
        ...
        if (unverified) {
          // Desinscrever fator não verificado pendente para criar um limpo
          await supabase.auth.mfa.unenroll({ factorId: unverified.id });
        }

        const { data: enrollData, error: enrollError } = await supabase.auth.mfa.enroll({
          factorType: "totp",
          issuer: "Grupo J Diretoria",
          friendlyName: `Admin (${user.email || "Diretoria"})`
        });
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8 space-y-2">
          ...
          <p className="text-xs text-slate-500 max-w-xs mx-auto mt-1">
            Proteção de autenticação reforçada em dois fatores para o painel administrativo.
          </p>
        </div>
        ...
        <Card variant="elevated" className="border border-slate-200/80 shadow-xl bg-white">
          {mode === "checking" && (
            <CardContent className="p-8 text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 font-medium">Verificando status de segurança do usuário...</p>
            </CardContent>
          )}
          ...
          {mode === "verify" && (
            <>
              <CardHeader className="p-6 pb-2 border-b-0">
                ...
                <CardDescription>
                  Digite o código de 6 dígitos gerado pelo seu autenticador corporativo (Google Authenticator, 1Password, etc.).
                </CardDescription>
              </CardHeader>
            ...
            <>
              <CardHeader className="p-6 pb-2 border-b-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant="warning">Ativação Obrigatória</Badge>
                </div>
                <CardDescription>
                  Escaneie o código QR abaixo usando seu aplicativo autenticador favorito (Google Authenticator, Microsoft Authenticator, Authy ou 1Password).
                </CardDescription>
              </CardHeader>
                  ...
                  {qrCodeSvg && (
                    <div className="flex flex-col items-center justify-center p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                      ...
                      <p className="text-[11px] text-slate-400 mt-2">
                        Escaneie com a câmera do seu app autenticador
                      </p>
                    </div>
                  )}
