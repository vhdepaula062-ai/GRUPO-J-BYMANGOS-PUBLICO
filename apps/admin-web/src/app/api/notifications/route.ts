...
export async function GET(request: NextRequest) {
  ...
  let query = ctx.db.from("operational_notifications").select("id,title,message,severity,entity_type,created_at").eq("audience","admin").order("created_at",{ascending:false}).order("id",{ascending:false}).limit(51);
...
export async function POST(request: NextRequest) {
  ...
  let query = ctx.db.from("operational_notifications").select("id").eq("id",id).eq("audience","admin");
  if (ctx.organizationId) query = query.eq("organization_id",ctx.organizationId);
