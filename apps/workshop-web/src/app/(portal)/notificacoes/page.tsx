...
export default async function NotificationsPage(){
  ...
  let query=db.from("operational_notifications").select("id,title,message,severity,entity_type,created_at").eq("audience","workshop").order("created_at",{ascending:false}).order("id",{ascending:false}).limit(51);
  const workshop = await getMyWorkshop();
  if(!workshop) return <NotificationCenter initialError="Acesso não autorizado." />;
  query=query.eq("organization_id",workshop.organization.id);
